...
export default function JobAttachmentLink({ jobId, attachment }: {
  ...
  if (!file) return null;
  return (
    <a href={`/api/jobs/${jobId}/attachment`} className="mt-4 flex items-center gap-2 rounded-xl border border-blue-100 bg-blue-50 p-3 text-sm font-semibold text-blue-700 hover:bg-blue-100">
      <Paperclip size={18} className="shrink-0" />
      <span className="min-w-0 break-words">Download supporting document: {file.name}</span>
