    ...
    <form onSubmit={submit} className="space-y-5">
      ...
      <div>
        ...
        <input
          ...
          placeholder="e.g. Community Programme Coordinator"
        />
      </div>
      <div>
        <label className="mb-1.5 block text-sm font-semibold text-slate-700">
        ...
        <textarea
          ...
          placeholder="Describe the responsibilities, outcomes and experience needed."
        />
      </div>
      <div>
        ...
        <div className="flex flex-wrap gap-2">
          {JOB_CATEGORIES.map((category) => (
            <button
              ...
              className={`rounded-full border px-3 py-1.5 text-xs font-semibold ${categories.includes(category) ? "border-blue-600 bg-blue-600 text-white" : "border-slate-200 text-slate-600"}`}
            >
              {category}
            </button>
          ))}
      ...
      <div className="grid gap-4 sm:grid-cols-2">
        ...
        <label className="text-sm font-semibold text-slate-700">
          ...
          <select
            ...
            onChange={(event) =>
              setPayNegotiable(event.target.value === "discuss")
            }
            className="mt-1.5 w-full rounded-xl border border-slate-200 px-3 py-3 text-sm font-normal"
          >
        ...
        <div className="grid gap-4 sm:grid-cols-2">
          ...
          <label className="text-sm font-semibold text-slate-700">
            ...
            <select
              value={payCadence}
              onChange={(event) =>
                setPayCadence(event.target.value as "weekly" | "monthly")
              }
              className="mt-1.5 w-full rounded-xl border border-slate-200 px-3 py-3 text-sm font-normal"
            >
      ...
      {!payNegotiable && (
        <label className="text-sm font-semibold text-slate-700">
          ...
          <select
            ...
            onChange={(event) =>
              setSettlementMode(event.target.value as "managed" | "direct")
            }
            className="mt-1.5 w-full rounded-xl border border-slate-200 px-3 py-3 text-sm font-normal"
          >
        ...
        <div className="flex overflow-hidden rounded-xl border border-slate-200 text-sm">
          {(["online", "hybrid", "in_person"] as const).map((mode) => (
            <button
              ...
              className={`flex-1 px-3 py-2.5 capitalize ${workMode === mode ? "bg-blue-600 text-white" : "text-slate-600"}`}
            >
              {mode.replace("_", " ")}
            </button>
          ))}
      ...
      {workMode === "hybrid" && (
        <label className="text-sm font-semibold text-slate-700">
          ...
          <input
            ...
            max="6"
            value={daysOnSite}
            onChange={(event) => setDaysOnSite(event.target.value)}
            className="mt-1.5 w-full rounded-xl border border-slate-200 px-3 py-3 text-sm font-normal"
          />
        </label>
      )}
      ...
      >
        {loading ? "Posting role..." : "Post Organisation role"}
      </button>
    </form>
  );
}
